...
public:
	...
	DeleteFrame(LinkedList <long, Frame*>* frameList, PortalSystem2 *psAdapter, Array<Subject> * mainDB) : Frame(FRAME_DEL, frameList, psAdapter, mainDB)
	{
		//0번 : 배경 오브젝트
		...
		//3번 : 삭제 메뉴 오브젝트
		this->add(TextObject(this->width / 3, 0, this->width / 6, this->height / 6, COL_BUTTONSELLECTED_RG, COL_BUTTONSELLECTED_RG, COL_BUTTONSELLECTED_B, "과목 삭제", strlen("과목 삭제"), COL_TEXTNOMAL_RGB, COL_TEXTNOMAL_RGB, COL_TEXTNOMAL_RGB, SIZE_TEXT_W, SIZE_TEXT_H, true));
		//4번 : 보고서 출력 오브젝트
		...
		//10번 : 삭제 오브젝트
		this->add(TextObject(this->width * 10 / 16, this->height * 5 / 12, this->width / 8, this->height / 12, COL_BUTTON_RGB, COL_BUTTON_RGB, COL_BUTTON_RGB, "삭제", strlen("메인"), COL_TEXTNOMAL_RGB, COL_TEXTNOMAL_RGB, COL_TEXTNOMAL_RGB, SIZE_TEXT_W, SIZE_TEXT_H, true));
	}
	...
	virtual void keyEvent(HWND hWnd, char c[3], bool finish) {
		...
		if (finish) {
			...
			case VK_RETURN: 
			{
				...
				if (!(subject == Subject())) {
					this->psAdapter->remove(*this->mainDB, subject);
					this->animationClose(hWnd, this->otherFrame(FRAME_MAIN), true);
				}
				else {
